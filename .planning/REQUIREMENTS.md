...
- [ ] **FOUND-01**: Database migrated from SQLite to PostgreSQL with Alembic migrations
- [ ] **FOUND-02**: Full-text search uses PostgreSQL tsvector + GIN index (replacing FTS5)
- [ ] **FOUND-03**: Environment config supports production PostgreSQL connection
...
- [ ] **PIPE-04**: SAM.gov contract opportunities ingested (with registered API key, incremental design)
- [ ] **PIPE-05**: Pipeline monitoring detects stale data (no update in 48h triggers alert)
- [ ] **PIPE-06**: Pipeline logs ingestion stats (records added/updated/failed per run)
...
- [ ] **QUAL-02**: Agency names/codes normalized across all sources
- [ ] **QUAL-03**: Duplicate opportunities detected and merged across sources
- [ ] **QUAL-04**: LLM-powered categorization tags opportunities by topic/sector
- [ ] **QUAL-05**: Date fields consistently ISO 8601 across all sources
- [ ] **QUAL-06**: Award amounts validated (floor <= ceiling, no negative values)

### State Data

- [ ] **STATE-01**: Scraping infrastructure for state grant portals using Scrapling
- [ ] **STATE-02**: At least 5 state portals scraped and normalized into unified schema
- [ ] **STATE-03**: Per-state legal review completed (ToS/robots.txt/open-data check)
